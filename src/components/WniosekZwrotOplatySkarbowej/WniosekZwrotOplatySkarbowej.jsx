...
import { fillPdf } from "../utils/wniosekZwrotOplatySkarbowej.js";
import css from "./WniosekZwrotOplatySkarbowej.module.css";
...
export default function WniosekZwrotOplaty() {
  const { t } = useTranslation("wniosekZwrotOplatySkarbowej");

  const [savedValues, setSavedValues] = useState(() => {
    const saved = localStorage.getItem("wniosekZwrotOplaty");
  ...
  const handleFieldChange = (field, value, setFieldValue) => {
    ...
    localStorage.setItem("wniosekZwrotOplaty", JSON.stringify(updated));
  };
  ...
  const validationSchema = Yup.object().shape({
    ...
    imieNazwisko2: Yup.string().when("formaZwrotu", {
      is: "na adres",
      then: (schema) =>
        schema
          .matches(polishRegex, t("schema.polishRegex"))
    ...
    dokladnyAdress: Yup.string().when("formaZwrotu", {
      is: "na adres",
      then: (schema) => schema.required(t("schema.required")),
      otherwise: (schema) => schema.notRequired(),
    }),
    ...
    ulica: Yup.string()
      .required(t("schema.required")),
    numerUlicy: Yup.string().required(t("schema.required")),
        ...
        {({ values, setFieldValue, resetForm }) => (
          <Form className={css.form}>
            ...
            <div className={css.single}>
              ...
              >
                <option value="">{t("options.default")}</option>
                <option value="340 zł.">{t("options.340")}</option>
                <option value="440 zł.">{t("options.440")}</option>
                <option value="640 zł.">{t("options.640")}</option>
              </Field>
            ...
            <FieldBlock
              ...
              placeholder="Wniosek o wydanie zezwolenia został wycofany przed rozpoczęciem postępowania."
              {...{ values, setFieldValue, handleFieldChange }}
            />
