...
export async function fillPdf(form) {
  const existingPdfBytes = await fetch("/wniosek-o-zwrot-opłaty-skarbowej-za-KP.pdf").then(res => res.arrayBuffer());
  ...
  // kwota
  if (form.kwota === "100 zł.") {
    setField("kwota", "100");
  } else if (form.kwota === "50 zł.") {
    setField("kwota", "50");
  }
  ...
  saveAs(new Blob([pdfBytes], { type: "application/pdf" }), "wypelniony_formularz_zwrot-opłaty-za-KP.pdf");
}
